                    ...
                    <div class="d-flex guttar-15px">
                        ...
                        <div class="fake-class">
                            <form action="{{ route('admin.ajax.stages.active') }}" method="POST">
                                @csrf
                                <a href="javascript:void(0);" class="update_stage" data-type = "active_stage" data-id="{{$ico->id}}" class="btn btn-icon btn-sm btn-{{(get_setting('actived_stage') == $ico->id)?'danger disabled':'danger-alt'}}"><em class="fas fa-star"></em></a>
